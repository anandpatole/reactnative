...
export default class MyRing extends Component {
	constructor(props) {
		super(props);
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (
			...
			MyRing.rearrangeItems(0, _state, props);
			return _state;
		}
		return null;
	}
	...
	addPenGesture() {
		this.panResponder = PanResponder.create({
			...
			onPanResponderMove: (evt, gestureState) => {
				...
				let items = MyRing.rearrangeItems(angle, state, props);
				this.setState({ items });
			},
	...
	rotateItems(angleToRotate, sign) {
		...
		if (Math.abs(angleToRotate) <= ROTATION_RATE) {
			items = MyRing.rearrangeItems(angleToRotate, state, props);
		} else {
			items = MyRing.rearrangeItems(
				ROTATION_RATE * sign,
				state,
				props
			);
	...
	static resetItemDimension(item, state, props) {
		...
		let c = MyRing.scalingCoefficient(item, state);
		let w = width * c;
		let h = height * c;
	...
	static rearrangeItems(toAngle, state, props) {
		...
		items.forEach((item) => {
			...
			MyRing.resetItemDimension(item, state, props);
		});
		return items;
	}
}
